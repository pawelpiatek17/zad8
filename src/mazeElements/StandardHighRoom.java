...
import visitors.StandardHighRoomVisitor;
import visitors.Visitor;

public class StandardHighRoom implements HighRoom {
	ArrayList<RoomPart> parts;
	private StandardWall wall;
	private boolean lightStatus = false;
	Lamp lamp;
	public StandardHighRoom(double x, double y, double width, double heigth) {
		parts = new ArrayList<>();
		wall = new StandardWall(x, y, width, heigth);
		parts.add(wall);
	...
	public void accept(BaseVisitor acyclicDrawVisitor, Pane pane) {
		if (acyclicDrawVisitor instanceof StandardHighRoomVisitor) {
			((StandardHighRoomVisitor) acyclicDrawVisitor).visit(this,pane);
			pane.getChildren().add(lamp);
			lamp.toFront();
		}
	}
